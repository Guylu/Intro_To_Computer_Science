...
def check_input_args(args1, args2, args3, args4):
    ...
    # directions
    if args1 and args2 and args3 and args4:  # should be exactly 4 arguments
        # $$$$$$$$$$$$$$$$4$$$$$
...
def match(word, matrix, row_index, column_index, direction):
    ...
    for offset_two in range(len(word)):  # loop in the words length-to
        ...
        return_value += 1
        if direction == UP_TO_DOWN_LEFT or direction == DOWN_TO_UP_RIGHT:
...
def vals_for_loop(word_len, matrix_getlengh0, matrix_getlengh1 , direction):
    ...
    cheking mechanism will be. because my code is designed in a way that a
    ...
    """
    VALUES_FOR_LOOP=[[0, matrix_getlengh0, 0, matrix_getlengh1 - word_len +1]
                   ,[0, matrix_getlengh0 - word_len + 1, 0, matrix_getlengh1],
     [0, matrix_getlengh0 - word_len + 1, 0, matrix_getlengh1 - word_len + 1],
     [matrix_getlengh0 - 1, word_len - 2, 0, matrix_getlengh1 - word_len + 1]]
...
def find_words_in_matrix(word_list, matrix, directions):
    ...
    for word in word_list:  # all words
        for dir in directions:  # all direction given
            ...
            if found:  # if not 0
                words_found_in_matrix.append((word, found))  # tuple
                # structure = (word, number of times in matrix in direction)
...
def run_puzzle(args1, args2, args3 ,args4):
    ...
    """
    check_input_args(args1, args2, args3, args4)#########################3
    ...
    print("Placing results in output file:", output_path)
