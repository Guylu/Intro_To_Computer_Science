...
def test_board_str_different_representations():
    ...
    a = board.add_car(Car("R", 2, (0, 0), HORIZONTAL))
    assert a
    board_strs.add(str(board))
    y = board.move_car("R", MOVE_RIGHT)
    assert y
    board_strs.add(str(board))
...
def test_possible_moves_works():
    ...
    def car_moves(car_name):
        lst = []
        for name, move, _desc in board.possible_moves():
            if car_name == name:
                lst.append(move)
        return sorted(lst)
