...
class GameRunner:
    ...
    def _game_loop(self):
        ...
        for tor in self.__torpedos:
            ...
            if smashedroid:
                ...
                self.tor_smash(tor, smashedroid)
    ...
    def tor_smash(self, tor, smashedroid):
        self.__screen.unregister_torpedo(tor)
        self.__torpedos.remove(tor)

        if smashedroid.get_size() <= 1:
            return
    ...
    def torpedo_move(self):
        ...
        for tor in self.__special_shoots:
            tor.recalibrate(*asteroid.get_location())
            self.move_object(tor)
    ...
    def shoot(self, is_special=False):
        if ((len(self.__torpedos) == 10 and not is_special) or
                (len(self.__special_shoots) == 5 and is_special)):
            return
